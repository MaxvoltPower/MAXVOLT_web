    ...
    <>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        ...
        <select
          ...
          className="max-w-xs px-4 py-2.5 rounded-xl border-[1.5px] border-dark-border bg-dark-muted cursor-pointer"
        >
          ...
          <table className="w-full text-sm">
            <thead className="bg-dark-muted">
              <tr>
                ...
                <th className="px-3 py-3 text-left text-xs font-bold uppercase">Brand</th>
                ...
                <th className="px-3 py-3 text-left text-xs font-bold uppercase">Price</th>
                <th className="px-3 py-3 text-left text-xs font-bold uppercase">Stock</th>
            ...
            <tbody>
              {filtered.map((p) => (
                <tr key={p._id} className="border-t border-dark-border hover:bg-accent/5">
                  ...
                  <td className="px-3 py-3 text-xs">{CATEGORY_LABELS[p.category] || p.category}</td>
                  <td className="px-3 py-3">{formatPrice(p.discountedPrice || p.price)}</td>
                  <td className="px-3 py-3">
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Input label="Brand" name="brand" required value={formData.brand} onChange={handleChange} />
            <Input label="Model" name="model" required value={formData.model} onChange={handleChange} />
          </div>

          <Select label="Category" name="category" required value={formData.category} onChange={handleChange}>
            {Object.entries(CATEGORY_LABELS).map(([key, label]) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Input label="MRP (₹)" name="price" type="number" value={formData.price} onChange={handleChange} />
            <Input label="Discounted Price (₹)" name="discountedPrice" type="number" value={formData.discountedPrice} onChange={handleChange} />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Input label="VA (for inverters/UPS)" name="va" value={formData.va} onChange={handleChange} />
            <Input label="Stock Quantity" name="stock" type="number" min="0" value={formData.stock} onChange={handleChange} />
          </div>
